...
(function () {
    ...
    // eateryData is provided via wp_localize_script in functions.php
    const logoUrl = (typeof eateryData !== 'undefined' && eateryData.logoUrl)
        ? eateryData.logoUrl
        : '/wp-content/themes/eatery-essentials/assets/images/logo-blue_green.png';
  ...
  <div id="spec-modal" role="dialog" aria-modal="true" aria-label="Product Spec Sheet"
    ...
    <div id="spec-modal-inner"
      ...
      <div style="background:linear-gradient(135deg,#16a34a,#0891b2);padding:1.75rem 2rem;display:flex;align-items:center;justify-content:space-between;">
        <div>
          <img id="ss-logo" src="${logoUrl}" alt="Eatery Essentials"
    ...
    function scrapeProduct() {
        ...
        const sku = (() => {
            const allDescs = document.querySelectorAll('.product-desc, .specs-table tr');
            for (const d of allDescs) {
                if (d.textContent.includes('SKU:')) return d.textContent.trim();
                if (d.querySelector('th')?.textContent.includes('Item No')) return 'SKU: ' + d.querySelector('td').textContent.trim();
            }
            return '';
        })();
        ...
        return { title, cat, desc, sku, imgSrc, rows };
    }

    /* ─── Populate and open modal ─────────────────────────────────── */
    function openModal() {
        const { title, cat, desc, sku, imgSrc, rows } = scrapeProduct();
